...
class Evaluator:
	...
	def satisfability(self, formula, mode = 1):
		...
		for evaluation in range(numberOfEvaluations):
			...
			remainingValues = [False] * (numberOfAtoms - len(evaluationValues)) # Getting the remaining values to match with atoms

			definitionValues = remainingValues + evaluationValues # Values for the definition dictionary

			definition = dict(zip(atoms, definitionValues)) # Building a dictionary with atoms as Keys and definitionValues as Values
